...
botBreak = '' -- change value to player if move block to player if not change to empty or nil
displayBlockID = 1422
blocksID = "340"
seedsID = "341"
    ...
    if gaia > 0 then
        for i = 1, gaia do
            for l=1,7 do 
                ...
                if(gaiaPos[i].x >= 50) then
                    findPath(gaiaPos[i].x - 1, gaiaPos[i].y - 3 - l)
                    faceSide("right")
                else
                    findPath(gaiaPos[i].x + 1, gaiaPos[i].y - 3 - l)
                    faceSide("left")
    ...
    if ut > 0 then
        for i = 1, ut do
            for l=1,2 do 
                ...
                sleep(2000)
                if botBreak == nil or botBreak == '' then
                    ...
                    for _,player in pairs(getPlayers()) do
                        if player.name == botBreak then
                            findPath(player.pos.x+1, player.pos.y)
                            faceSide("left")
                            sleep(2000)
                            dropItem(blocksID, 200)
                            sleep(2000)
